...
function Codificar() {
  ...
  // Aplica regras de complemento automaticamente.
  useEffect(() => {
    if (step !== "complemento") return;
    const regra = complementoForcado(artigo);
    if (regra === "posicao-00") {
      setComplementoTipo("posicao");
      setComplemento("00");
    } else if (regra === "posicao-any") {
      setComplementoTipo("posicao");
    ...
    <Shell>
      ...
      {step === "complemento" && (
        <StepCard title="Escolher Complemento">
          {(() => {
            const regra = complementoForcado(artigo);
            const showPosicao = true; // posição está sempre disponível
            const showPais = regra === "both";
            const aviso =
              regra === "posicao-00"
                ? "Este artigo exige complemento de POSIÇÃO 00"
                : regra === "posicao-any"
                  ? "Este artigo aceita apenas complemento de POSIÇÃO"
                  : null;
            return (
                ...
                {showPosicao && (
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {POSICOES.map((p) => {
                        const disabled = regra === "posicao-00" && p !== "00";
                        return (
                          <OptionButton
                            key={p}
                            disabled={disabled}
