...
export type ComplementoRegra = "posicao-00" | "posicao-any" | "both";

export function complementoForcado(artigo: string): ComplementoRegra {
  // Produto acabado: obrigatório posição 00
  if (artigo === "0") return "posicao-00";
  // Conjunto e Peça: apenas posição (qualquer)
  if (artigo === "1" || artigo === "2") return "posicao-any";
  // Restantes (incluindo esquemas 3/4/5 e documento 8): posição ou país
  return "both";
}
